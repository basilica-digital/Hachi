...
pub unsafe fn fold_witness_row_jolt(z: &mut [i32], id: &[u8], c: &[i32], batch: usize){
    ...
    for i in 0..(1<<16){
        ...
        for j in 0..4{
            // c 
            for k in 0..128{
                let v256 = _mm256_loadu_si256(c_ptr.add(i*128 + k));
    ...
    let m = (1i64 << 31) - 19;
    for j in 0..batch {
        let res_ptr = res[j][0].as_ptr() as *const i64;
        for i in 0..1024 {
            let val1 = *res_ptr.add(i);
            let val2 = *res_ptr.add(i + 1024);
            z[i + j * 1024] = ((val1 - val2).rem_euclid(m)) as i32;
        }
    }
}

#[inline(never)]
pub unsafe fn fold_witness_jolt(z: &mut Vec<i32>, c: &[i32], s: &[u64], batch: usize) {
    // id size: 2 + 16 + 8
    let mut id = vec![0u8; (1<<26)];
